<?php
    ...
    function getCoursesByFilters($categoryId, $keyword, $page) {
        ...
        if(!empty($categoryId)){
            $query = "FROM kurs_kategori kc INNER JOIN kurslar k ON kc.kurs_id=k.id WHERE kc.kategori_id=" . intval($categoryId) . " AND k.onay=1";
        } 
        else{
        ...
        if(!empty($keyword)){
            $keyword = mysqli_real_escape_string($baglanti, $keyword);
        ...
        if(!$count_data){
            ...
            return array("total_pages" => 0, "data" => false);
        }
    ...
    function getCoursesByCategoriesId(int $id){
        include "ayar.php";
    ...
    function createCourse(string $baslik, string $altBaslik, string $aciklama, string $resim, int $begeniSayisi = 0, int $yorumSayisi = 0, int $onay = 0){
        include "ayar.php";

        $query = "INSERT INTO kurslar(baslik, altBaslik, aciklama, resim, begeniSayisi, yorumSayisi, onay) VALUES (?,?,?,?,?,?,?)";
        $stmt = mysqli_prepare($baglanti,$query);

        mysqli_stmt_bind_param($stmt, 'ssssiii', $baslik, $altBaslik, $aciklama, $resim, $begeniSayisi, $yorumSayisi, $onay);
        mysqli_stmt_execute($stmt);
        mysqli_stmt_close($stmt);
